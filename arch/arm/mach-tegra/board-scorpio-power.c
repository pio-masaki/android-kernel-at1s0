...
int __init scorpio_charge_init(void)
{
...
static struct i2c_board_info __initdata scorpio_regulators[] = {
	{
...
static void scorpio_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void scorpio_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data scorpio_suspend_data = {
	/*
	...
	.corereq_high	= false,
	.sysclkreq_high	= true,
	.board_suspend = scorpio_board_suspend,
	.board_resume = scorpio_board_resume,
};

int __init scorpio_regulator_init(void)
{
	...
	if (minor == 3 && !(tegra_spare_fuse(18) || tegra_spare_fuse(19)))
		scorpio_suspend_data.suspend_mode = TEGRA_SUSPEND_LP1;
	...
	i2c_register_board_info(4, scorpio_regulators, 1);

//	regulator_has_full_constraints();

	tegra_init_suspend(&scorpio_suspend_data);

	return 0;
}

static char *scorpio_battery[] = {
	"battery",
};

static struct gpio_charger_platform_data scorpio_charger_pdata = {
	.name = "ac",
	.type = POWER_SUPPLY_TYPE_MAINS,
	.gpio = AC_PRESENT_GPIO,
	.gpio_active_low = 1,
	.supplied_to = scorpio_battery,
	.num_supplicants = ARRAY_SIZE(scorpio_battery),
};

static struct platform_device scorpio_charger_device = {
	.name = "gpio-charger",
	.dev = {
		.platform_data = &scorpio_charger_pdata,
	},
};

int __init scorpio_charger_init(void)
{
	tegra_gpio_enable(AC_PRESENT_GPIO);
	platform_device_register(&scorpio_charger_device);
	return 0;
}

static int __init scorpio_pcie_init(void)
{
	int ret;
...
late_initcall(scorpio_pcie_init);
