...
static uhsic_powerup_pmc_wake_detect(struct tegra_usb_phy *phy)
{
	unsigned long val;
	void __iomem *pmc_base = IO_ADDRESS(TEGRA_PMC_BASE);
...
static int ulpi_phy_power_on(struct tegra_usb_phy *phy, bool is_dpd)
{
    ...
    /* Enhance USB phy driving strength 7.4%,
     * Reference to usb3315 datasheet p.60.
     */
	ret = otg_io_write(phy->ulpi, 0x40, 0x31);
#endif
#ifndef CONFIG_MACH_SCORPIO
